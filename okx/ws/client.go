...
func (b *OKXWS) Subscribe(channel string, args []string) {
	if channel != "tickers" {
		...
		case 1:
			b.Subscribe2(channel, args[0])
		case 2:
			b.Subscribe3(channel, args[0], args[1])
		case 3:
			b.Subscribe4(channel, args[0], args[1], args[2])
		default:
			log.Printf(`
				{
					"Status" : "Error",
					...
					"File": "client.go",
					"Functions" : "b *OKXWS) Subscribe(channel string, args []string) ",
					"Exchange" : "OKX",
					"Data" : [%v, %v],
			...
			log.Printf(`
				{
					...
					"Exchange" : "OKX",
					"Data" : [%v, %v],
					"Comment" : "Слишком много аргументов (за один раз можно подписаться только на одну пару)"
				}`, channel, args)
			log.Fatal()
		}
	}
